...
def main():
    ...
    while rerouting:
        ...
        for res in traci.person.getTaxiReservations(1):
            # print("Teleporting: ", traci.simulation.getStartingTeleportIDList())
            ...
            if not res_values.vehicle:
                if step >= res_values.max_processing:
                    ...
                    for person in res_values.persons:
                        ...
                        # ADDED: Remove the person entirely to free memory
                        try:
                            traci.person.remove(person)
                        except traci.TraCIException:
                            pass  # Person may already be gone
                else:
                ...
                if traci.vehicle.getStops(veh_id):
                    for taxi_stop in traci.vehicle.getStops(veh_id):
                        ...
                        for sub_stop in sub_stops:
                            m = re.search(r'\(([^)]+)\)', sub_stop)
                            if not m:
                                continue
                            pid = m.group(1).strip().strip('"').strip("'")
                            current_route.append(pid)
    ...
    WAIT_PERSONS_MAX = 1200  # seconds, adjust as needed
    max_wait_steps = max(1, int(WAIT_PERSONS_MAX / options.sim_step))
    wait_steps = 0
    while traci.person.getIDCount() > 0 and wait_steps < max_wait_steps:
        if options.verbose:
            print(f"Waiting for {traci.person.getIDCount()} person(s) to leave (sim time {step})")
        traci.simulationStep(step)
        step += options.sim_step
        wait_steps += 1
